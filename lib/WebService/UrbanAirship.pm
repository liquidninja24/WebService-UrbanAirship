...
sub new {
  ...
  foreach my $key (qw(application_key application_secret application_master_secret)) {
    die "missing argument: $key"
      unless $args{$key};
  }

  my $self = {_master_secret => delete $args{application_master_secret},
              _secret        => delete $args{application_secret},
              _key           => delete $args{application_key},
              _api_uri       => delete $args{api_uri},
             };
...
sub ua {
  ...
  $headers->header('Accept' => 'application/vnd.urbanairship+json; version=3');

  # set the authentication headers here
  $headers->authorization_basic($self->{_key}, $self->{_master_secret});
...
sub _api_uri {
  ...
  return URI->new($self->{_api_uri} || 'https://go.urbanairship.com/');
}
...
sub register_device {
  ...
  # this API requires the secret key, not the master secret key
...
sub _craft_payload {
  ...
  my $ios = delete $args{ios};
  my $android = delete $args{android};
  my $blackberry = delete $args{blackberry};
  my $wns = delete $args{wns};
  my $mpns = delete $args{mpns};

  my $payload = {};

  $payload->{alert} = $alert if $alert;
  $payload->{ios} = $ios if $ios;
  $payload->{android} = $android if $android;
  $payload->{blackberry} = $blackberry if $blackberry;
  $payload->{wns} = $wns if $wns;
  $payload->{mpns} = $mpns if $mpns;
...
WebService::UrbanAirship - routines for Urban Airship Apple Push Notification service

=head1 SYNOPSIS

  # create the object
  my $o = WebService::UrbanAirship->new(application_key           => 'C9mvZ******************8QGW',
                                        application_secret        => 'DQvNtylF***************MgVG',
                                        application_master_secret => 'HGrBg37****************ylFA');
...
WebService::UrbanAirship contains useful routines for using the
...
instantiate a new WebService::UrbanAirship object.
...
=item application_master_secret

the Application Master Secret assigned to your application.
this argument is required.
